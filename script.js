...
function ageCalculate(){
    let today = new Date(); // Date object created with currnt date and time
    
    let inputDate = new Date(document.getElementById('date-input').value);  
    let birthMonth,birthDate,birthYear;

    let birthDetails = { // here creating object of birthDetails with details like date and month and year
        date:inputDate.getDate(),
        month:inputDate.getMonth() + 1,// inputDate.getMonth() + 1: Gets the month of the year (0–11) from the user’s birth date. Adding 1 adjusts the value to a 1–12 range (January = 1, December = 12).
    ...
    if(
        ...
        (birthDetails.date > currentDate && birthDetails.month == currentMonth && birthDetails.birthYear == currentYear)
    ){
        alert("Not Born Yet");
        displayResult("-","-","-") //Calls a function displayResult with arguments "-", "-", and "-".
    }
